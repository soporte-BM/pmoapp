...
export function renderEntryForm(container) {
    ...
    const addProfessionalRow = () => {
        ...
        div.innerHTML = `
            ...
            <input type="number" name="pro_hours" placeholder="Horas" class="form-input" style="flex:1" min="0" step="0.01" required>
    ...
    form.addEventListener('submit', async (e) => {
        ...
        names.forEach((name, i) => {
            if (name) {
                const parsedHours = parseFloat(hours[i]);
                professionals.push({
                    name,
                    hours: isNaN(parsedHours) ? 0 : Math.round(parsedHours * 100) / 100,
                    rate: Number(rates[i])
                });
            }
        });
