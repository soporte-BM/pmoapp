...
export async function renderProjects(container) {
    ...
    const render = () => {
        ...
        const filteredProjects = projects.filter(p => {
            const matchesStatus = currentFilter === 'Todos' || p.status === currentFilter;
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 15px;">
                    <div style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                        ...
                        <div style="display: flex; gap: 8px; align-items: center;">
                            <input type="text" id="projectSearchInput" class="form-input" placeholder="Buscar por código o nombre..." value="${searchQuery}" style="width: 240px;" />
                            <button id="btn-search-project" class="btn-secondary" style="padding: 6px 14px;">🔍 Buscar</button>
                            <button id="btn-clear-search" class="btn-secondary" style="padding: 6px 14px; background: #f3f4f6; color: #374151; border: 1px solid #d1d5db;">Limpiar</button>
                        </div>
                    </div>
    ...
    const attachEvents = () => {
        ...
        const btnSearch = document.getElementById('btn-search-project');
        ...
        if (btnSearch) {
            btnSearch.addEventListener('click', () => {
                ...
                render();
                const inputAfter = document.getElementById('projectSearchInput');
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 5px;">
                            ${entry.professionals.map((p, i) => `
                                <div style="display: flex; align-items: center; gap: 10px;">
                                    ...
                                    <input type="number" class="form-input edit-prof-hour" data-index="${i}" value="${p.hours}" style="width: 80px;" min="0" step="0.01">
                                </div>
                            `).join('')}
                        </div>
                    </div>
        ...
        document.getElementById('btn-save-edit').addEventListener('click', async () => {
            ...
            if (!isHistorical) {
                ...
                hourInputs.forEach(input => {
                    ...
                    const parsedH = parseFloat(input.value);
                    updatedPros[idx].hours = isNaN(parsedH) ? 0 : Math.round(parsedH * 100) / 100;
                });
            }
